...
public class Graph extends JPanel {
	...
	private double bigXTick = 1;
	private double bigYTick = 1;

	// Strokes
	private int axisStroke = 2;
	private int gridStroke = 1;

	// Total xRange of graph (E.g. xRange=10, start=-5, end=5)
	private double xRange = 10;
	private double yRange = xRange;
	...
	public void paintComponent(Graphics g) {
		...
		// Make sure bigTick is in correct ratio with x and y range
		bigXTick = xRange / numOfBigTicksInRange;
		bigYTick = yRange / numOfBigTicksInRange;

		// Go through each grid line
		for (int i = 0; i <= numOfBigTicksInRange; i++) {
			/* Number to be drawn on graph at respective tick */
			// String xNum = df.format(i*bigTick - xRange/2);
			// String yNum = df.format(xRange/2 - i*bigTick);

			// Map "i" to its position in range
			String xNum = df.format(map(i, 0, numOfBigTicksInRange, -xRange / 2, xRange / 2));
			String yNum = df.format(map(i, 0, numOfBigTicksInRange, yRange / 2, -yRange / 2));
	...
	public void setxRange(double xR, double yR) {
		xRange = xR;
		yRange = yR;
	}
	...
	private void connectPoints() {
		...
		for (int i = 0; i < points.length; i++) {
			if (i != 0) {
				int xPreviousLocationOnGraph = (int) map(points[i - 1].x, 0 - xRange / 2, xRange / 2, 0, w);
				int yPreviousLocationOnGraph = (int) map(points[i - 1].y, 0 - yRange / 2, yRange / 2, h, 0);
				int xLocationOnGraph = (int) map(points[i].x, 0 - xRange / 2, xRange / 2, 0, w);
				int yLocationOnGraph = (int) map(points[i].y, 0 - yRange / 2, yRange / 2, h, 0);
